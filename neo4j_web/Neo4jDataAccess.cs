...
public class Neo4JDataAccess : INeo4jDataAccess
{
    ...
    /// </summary>
    public async Task<IEnumerable<T>> ExecuteWriteTransactionAsync<T>(string query, IDictionary<string, object>? parameters = null) where T : new()
    {
        try
        {
            parameters = parameters == null ? new Dictionary<string, object>() : parameters;
            ...
            {
                IEnumerable<T> scalar = new List<T>();
                var res = await tx.RunAsync(query, parameters);
                scalar = await res.ToListAsync<T>();
                return scalar;
            });
    ...
    public async Task RunQueryAsync(string query, IDictionary<string, object>? parameters = null)
    {
        try
        {                
